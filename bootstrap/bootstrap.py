...
def python_postprocess(python_directory):
  python_path = os.path.join(python_directory, 'bin', 'python.exe')
  print(python_path)
  _check_call([python_path, '-m', 'pip', 'install', 'cryptography'])
...
def main(argv):
  ...
  if IS_WIN:
    python_postprocess(os.path.join(bootstrap_dir, 'python'))
